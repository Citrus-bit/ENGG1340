...
const string slotFiles[3] = { "slot1.dat","slot2.dat","slot3.dat" };
...
// Save Game
void saveGame(int slot) {
    ofstream f(slotFiles[slot]);
    f << difficulty << " " << playerHP << " " << playerGold << " " << waveNumber << "\n";
    f << towers.size() << "\n";
    for (auto& t : towers)
        f << t.row << " " << t.col << " " << t.damage << " " << t.isAOE << " " << t.rate << "\n";
}

bool loadGame(int slot) {
    ifstream f(slotFiles[slot]);
    if (!f) return false;
    f >> difficulty >> playerHP >> playerGold >> waveNumber;
    size_t n; f >> n;
    towers.clear(); monsters.clear(); projectiles.clear();
    for (size_t i = 0;i < n;++i) {
        Tower t; f >> t.row >> t.col >> t.damage >> t.isAOE >> t.rate;
        t.cooldown = 0; towers.push_back(t);
    }
    return true;
}
    ...
    while (true) {
        ...
        if (choice >= 2 && choice <= 4) {
            if (!loadGame(choice - 2)) {
                cout << "Load failed, press Enter to continue";cin.get(); continue;
            }
        }
        else if (choice == 1) {
        ...
        while (running && playerHP > 0) {
            ...
            do {
                ...
                if (post == 'S' || post == 's') {
                    cout << "Save slot (1-3): ";int sl;cin >> sl;cin.ignore(numeric_limits<streamsize>::max(), '\n');
                    if (sl >= 1 && sl <= 3) { saveGame(sl - 1);cout << "Saved\n"; }
                }
            } while (post == 'S' || post == 's');
